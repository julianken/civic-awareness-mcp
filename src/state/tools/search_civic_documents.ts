  ...
  if (results.length === 0) {
    ...
    if (!anyRow) {
      ...
      response.hint = input.jurisdiction
        ? `No documents for ${input.jurisdiction}. Try calling recent_bills first to warm the cache.`
        : "Local store is empty. Try calling recent_bills first to warm the cache.";
    }
  }
