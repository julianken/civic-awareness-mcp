...
const DB_PATH = process.env.CIVIC_STATE_DB_PATH ?? "./data/state.db";
...
export async function autoBootstrapIfNeeded(dbPath: string): Promise<void> {
  ...
  const store = openStore(dbPath, coreSqlPath, stateSqlPath);
...
async function main(): Promise<void> {
  logger.info("starting civic-state-mcp", { dbPath: DB_PATH });
