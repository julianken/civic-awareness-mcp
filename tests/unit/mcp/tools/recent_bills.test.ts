...
describe("recent_bills tool — projection (TTL-hit path)", () => {
  ...
  it("projects latest_action to {date, description} only, dropping upstream extras", async () => {
    ...
    upsertDocument(store.db, {
      kind: "bill",
      jurisdiction: "us-federal",
      title: "HB42 — shape pin",
      occurred_at: now,
      source: {
        name: "congress",
        id: "shape-pin",
        url: "https://congress.gov/bill/119/house-bill/42",
      },
      raw: {
        actions: [
          {
  ...
  it("rejects days above 365", async () => {
    await expect(handleRecentBills(store.db, { days: 366 })).rejects.toThrow();
  });

  it("accepts a small positive limit", () => {
    expect(() => RecentBillsInput.parse({ days: 7, limit: 5 })).not.toThrow();
  });

  it("accepts limit up to 500", () => {
    expect(() => RecentBillsInput.parse({ days: 7, limit: 500 })).not.toThrow();
  });

  it("rejects limit > 500", () => {
    expect(() => RecentBillsInput.parse({ days: 7, limit: 501 })).toThrow();
  });

  it("rejects limit=0", async () => {
    await expect(handleRecentBills(store.db, { days: 7, limit: 0 })).rejects.toThrow();
  });
  ...
  it("filters by session when session parameter is provided", async () => {
    upsertDocument(store.db, {
      kind: "bill",
      jurisdiction: "us-federal",
      title: "SB 1 — Eight-Ninety-Two",
      occurred_at: "2025-09-18T00:00:00Z",
      source: { name: "congress", id: "892-1", url: "https://congress.gov/bill/119/senate-bill/1" },
      references: [],
      raw: { session: "119", actions: [] },
    });
    upsertDocument(store.db, {
      kind: "bill",
      jurisdiction: "us-federal",
      title: "SB 99 — Eight-Ninety-One",
      occurred_at: "2024-06-01T00:00:00Z",
      source: {
        name: "congress",
        id: "891-99",
        url: "https://congress.gov/bill/118/senate-bill/99",
      },
      references: [],
      raw: { session: "118", actions: [] },
    });
  ...
  it("chamber=upper returns only upper-chamber bills (from_organization.classification)", async () => {
    ...
    upsertDocument(store.db, {
      kind: "bill",
      jurisdiction: "us-federal",
      title: "SB 10 — upper bill",
      occurred_at: now,
      source: {
        name: "congress",
        id: "upper-1",
        url: "https://congress.gov/bill/119/senate-bill/10",
      },
      references: [],
    ...
    upsertDocument(store.db, {
      kind: "bill",
      jurisdiction: "us-federal",
      title: "HB 20 — lower bill",
      occurred_at: now,
      source: {
        name: "congress",
        id: "lower-1",
        url: "https://congress.gov/bill/119/house-bill/20",
      },
      references: [],
  ...
  it("chamber=lower with all-upper bills returns empty + empty_reason=filter_eliminated_all", async () => {
    ...
    upsertDocument(store.db, {
      kind: "bill",
      jurisdiction: "us-federal",
      title: "SB 1 — upper only",
      occurred_at: now,
      source: {
        name: "congress",
        id: "fed-upper-1",
        url: "https://congress.gov/bill/119/senate-bill/1b",
      },
      references: [],
      raw: { from_organization: { classification: "upper" }, actions: [] },
    });
...
describe("recent_bills tool — R15 hydration path", () => {
  ...
  it("upstream failure with no cached data propagates the error", async () => {
    ...
    await expect(handleRecentBills(store.db, { days: 7 })).rejects.toThrow(/network down/);
