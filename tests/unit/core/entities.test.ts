...
describe("EXTERNAL_ID_PATHS + findEntityByExternalId", () => {
  ...
  it("findEntityByExternalId resolves by each known source", () => {
    ...
    expect(findEntityByExternalId(store.db, "openstates_person", "ocd-person/x")?.name).toBe(
      "OS Person",
    );
    expect(findEntityByExternalId(store.db, "fec_committee", "C00444444")?.name).toBe(
      "FEC Committee",
    );
