...
The `fetch_log` table grows unbounded as distinct query combos accumulate; at moderate use (~100 unique combos/day across 50 states × 3 sources) it reaches ~100k rows/year. The real cost isn't disk (~20 MB) but the chance of a long-stale row keeping `isFetchLogFresh` true after the cached documents it covers have been replaced. Operators should run `pnpm evict-fetch-log` periodically (e.g., monthly cron) to prune rows older than the configured cutoff. Default cutoff: 30 days; override with `--older-than-days <N>`.
